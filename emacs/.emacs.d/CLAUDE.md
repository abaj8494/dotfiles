...
- **Event descriptions** (`aj/gcal-include-body-in-desc`, advice on `org-gcal--get-time-and-desc`): the entry body *outside* the `:org-gcal:` drawer is merged into the pushed description. For `org-gcal-managed: org` entries the body **replaces** the drawer desc — the drawer is only an echo of the last push, so the old append behavior snowballed one duplicate copy of the body per re-push (i.e. per day, via the carry-forward). For gcal-managed entries the body is appended once, with a suffix check so re-posts don't re-append.
- **Drawer stays empty for chores** (`aj/gcal--suppress-drawer-desc`, `:filter-args` advice on `org-gcal--update-entry`): org-gcal's post-writeback would copy the posted description into the `:org-gcal:` drawer — a permanent echo of the body sitting right above it. For events on `aj/gcal-id-tasks` the advice strips `:description` from the response before the writeback, so chore drawers stay empty and the body is the single copy. Scoped to the Tasks calendar — gcal-managed J-calendar entries keep their drawer desc (their only copy).
- **tasks.org opt-out**: headings tagged `:noexport:` (own or inherited — `aj/heading-noexport-p`, daily-recurring.el) are excluded from dailies entirely: filtered out of the due-set (`aj/get-due-positions-for-date`, which also gates the child-subtree recursion) and skipped by the missed-task carry-forward (`aj/get-carryforward-tasks`).
- **Global push-chain mutex** (`aj/gcal--sweep-active-since` + `aj/gcal--sweep-active-p`): only ONE post chain may be in flight daemon-wide; `aj/gcal-sweep-daily-chores` and `aj/gcal-push-chore-at-point` back off with a message if another chain is active (flag auto-expires after 5 min so a dead chain can't wedge it; cleared at chain end). **Why**: the daily-open hook can fire twice for the same daily (midnight `daily-init` capture + an interactive visit), which used to launch two concurrent sweep chains. Each chain is internally sequential, but epg decrypts block in `accept-process-output` — a reentrancy window where the *other* chain's deferred callbacks run. Both chains hit `oauth2-auto.plist` via plstore, and plstore shares **one buffer per file** (`find-buffer-visiting`), so chain B's token-refresh `plstore-save`/`plstore-close` landed mid-decrypt of chain A and the next decrypt fed gpg clobbered ciphertext → `deferred error: (epg-error "Can't decrypt" "Exit")` (distinct from the loopback-pinentry cold-agent failure below, which the prewarm handles). Second layer: `aj/gcal-maybe-sweep-on-open` **coalesces** its 1s idle timer per buffer (cancel-and-replace), so a double hook fire schedules one sweep, not two.
